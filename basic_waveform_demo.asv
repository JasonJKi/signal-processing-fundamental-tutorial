...
py = 500; 
l =  1      % 2) we define the length of 1 unit to 1.
lx = 3     % 3) total length of x
ly = 3   % 4) total length of y
...
for i = 1:px
    ...
    for ii = 1:py
        x=(ii-1)*dy*ly;
        I1(i,ii) = sin((wx*x)/l+(wy*y)/l); %plugging circle equation in to complete the equation 
    end
end
...
% 2 Matlab
[X,Y] = meshgrid((0:lx/px:lx-1/px),(0:ly/py:ly-1/py)); %meshgrid replicates these 
